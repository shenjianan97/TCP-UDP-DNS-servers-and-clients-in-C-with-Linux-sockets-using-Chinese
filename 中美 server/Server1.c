...
int main(int argc, char *argv[])
{
    ...
    echoServAddr.sin_addr.s_addr =  inet_addr("127.0.0.4");//ip
...
//   dns->add_count = 0;
    ...
    for (;;)
    {
        ...
        if(dotsNum==0){//return the type of query
        ...
        else if(dotsNum==1){//if the query has the next level, education.china
            ...
            while(1) {
                ...
                if(!line)
                    break;
                if(findAddress(buf1, SLD, address) == 1){//address ->ip->addtional rdata
                    break;
                }
            }
            ...
            put2bytes(p,dns_h.id);
            //put2bytes(p,tagInt);
            put2bytes(p,generateDnsFlag(1,dns_h.opcode,0,dns_h.tc,dns_h.rd,dns_h.ra,dns_h.z,dns_h.rcode));
            put2bytes(p,1);
            put2bytes(p,0);
            put2bytes(p,0);
            put2bytes(p,1);
            ...
            char *add_domain=strcat(add_name,TLD);
            printf("addtitiona name:%s\n",add_domain);
            //answer
            struct DNS_RR_A additional;
            //unsigned char domainname[100] = "邮件.北邮.美国";
            additional.name = add_domain;
            printf("Name is %s\n", additional.name);
            printf("address is : %s\n", address);
