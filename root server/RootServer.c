...
int main(int argc, char *argv[])
{
    ...
    echoServAddr.sin_addr.s_addr =  inet_addr("127.0.0.3");
    ...
    unsigned char buf[ECHOMAX],*qname,*reader;
    qname = "www.baidu.com";
    int i , j , stop , s;
...
//   dns->add_count = 0;
    ...
    for (;;)
    {
        ...
        char buffer[100];
        char * TLD = getTLD(q0.name, buffer);
        printf("TLD:%s\n",TLD);
        ...
        while(1) {
            ...
            if(!line)
                break;
            if(findAddress(buf1, TLD, address) == 1){//address ->ip->addtional rdata
                break;
            }
        }
        if(strlen(address) == 0){
        ...
        put2bytes(p,dns_h.id);
        //put2bytes(p,tagInt);
    put2bytes(p,generateDnsFlag(1,dns_h.opcode,dns_h.aa,dns_h.tc,dns_h.rd,dns_h.ra,dns_h.z,dns_h.rcode));
        put2bytes(p,1);
        put2bytes(p,0);
        put2bytes(p,0);
        put2bytes(p,1);
       ...
       //unsigned char domainname[100] = "邮件.北邮.美国";
        ...
        additional.addr[0] = addr0;
        additional.addr[1] = addr1;
        additional.addr[2] = addr2;
        additional.addr[3] = addr3;
        printf("%d\n",addr3);
        ...
        /*additional.addr[0] = 127;
        additional.addr[1] = 0;
        additional.addr[2] = 0;
        additional.addr[3] = 1;*/
        encodeRR_A(p, &additional);

        //nS
        /*struct DNS_RR_CNAME nameServer;
        unsigned char domainName1[100] = "北邮.美国";
        unsigned char domainName2[100] = "邮件.北邮.中国";
        nameServer.name = domainName1;
        nameServer.type = T_CNAME;
        nameServer.class = 1;
        nameServer.ttl = 120;
        nameServer.data_len = 4;
        nameServer.cname = domainName2;

        encodeRR_CNAME(p, &nameServer);*/


        //echoClntAddr.sin_port = htons(53);
        //echoClntAddr.sin_addr.s_addr = inet_addr("192.168.50.36");

        //sendto(sock, buf, *p, 0, (struct sockaddr *)&echoClntAddr,sizeof(echoClntAddr));
        ...
        if ((sendto(sock, buf, (*p - initPointer), 0, (struct sockaddr *)&dest,sizeof(dest))) != recvMsgSize)
            printf("sendto() sent a different number of bytes than expected.\n");
